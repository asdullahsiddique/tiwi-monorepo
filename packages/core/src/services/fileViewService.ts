...
export async function getFileView(params: {
  ...
  try {
    ...
    const file = await fileRepo.getFile({ orgId: params.orgId, fileId: params.fileId });
    const summary = await artifactRepo.getFileSummary({ orgId: params.orgId, fileId: params.fileId });
    ...
    const downloadUrl = file
      ? await createPresignedGetUrl({ objectKey: file.objectKey, expiresInSeconds: 60 * 10 })
      : null;
