...
export type SemanticSearchResult = {
  ...
  chunks: SimilarChunk[];
  relatedFiles: Array<{ fileId: string; originalName: string; contentType: string }>;
};
...
export async function semanticSearch(params: {
  ...
  try {
    ...
    const context = chunks
      .slice(0, 8)
      .map((c, i) => `# Chunk ${i + 1} (fileId=${c.fileId}, chunkId=${c.chunkId}, score=${c.score})\n${c.text}`)
      .join("\n\n");

    const structuralContext = relatedFiles
      .map((f) => `- fileId=${f.fileId} name="${f.originalName}" type=${f.contentType} status=${f.status}`)
      .join("\n");
